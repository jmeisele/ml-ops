...
for i in range(len(data.data)):
    ...
    body = {"median_income_in_block": array[0],
            "median_house_age_in_block": array[1],
            "average_rooms": array[2],
            "average_bedrooms": array[3],
            "population_per_block": array[4],
            "average_house_occupancy": array[5],
            "block_latitude": array[6],
            "block_longitude": array[7]
            }
    # POST request to our API
    requests.post(url="http://localhost/api/model/predict", json=body)
